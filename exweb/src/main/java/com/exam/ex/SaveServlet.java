...
//	(3)서블릿컨텍스트 객체
//		-웹 애플리케이션 전체에서 1개의 서블릿컨텍스트 객체 생성
//		-서버(톰캣)가 시작될 때 생성, 서버가 종료될 때소멸 
...
//		동일한 웹사이트로 요청을 보낼때 쿠키를 요청헤더에 쿠키를 자동으로 포함(변경 가능)
//		- 만료기간을 설정하면, 웹브라우저가 종료되더라도 쿠키 값 유지 가능
//		- 웹브라우저에서 접근하여 사용 가능하기 때문에 보안상 위험 존재
//		- 쿠키의 이름과 값은 쉼표,세미콜론,공백 등 특수문자와 한글 등 비영어권 문자 사용 불가
//		일반적으로 쿠키이름은 영문자와 숫자만 사용, 쿠키값은 인코딩/디코딩하여 사용(한글값으로 저장해야할 경우)
//	(2)HTML5에서는 sessionStorage, localStorage,(key-value 쌍) indexedDB (db로 브라우저에 저장)도 사용 가능
...
public class SaveServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//check한 경우에 rval에 on이 들어감.
		
		//현재 요청을 보낸 사용자의 세션객체 가져오기 (없으면 생성)
		HttpSession session = req.getSession();
		session.setAttribute("nick", nval); //세션객체에 "nick"(속성이름)라는 이름으로 nval(속성값) 변수값을 저장
		
		//현재 웹 애플리케이션의 서블릿컨텍스트 객체 가져오기
		ServletContext context = getServletContext();
		context.setAttribute("nick", nval); //세션객체에 "nick"라는 이름으로 nval 변수값을 저장
		
		if("on".equals(rval)) {
			String enval = URLEncoder.encode(nval, "UTF-8"); //한글,특수문자 포함시 인코딩 필요
			Cookie c = new Cookie("nick", nval); //데이터 이름-값 을 담은 쿠키 생성
			c.setMaxAge(60*5); //쿠키 유효기간(초) 설정 (0은 즉시 삭제, 음수는 웹브라우저 종료시 삭제, 설정 안했을때 기본값은 -1)
//			c.setDomain("도메인"); //지정한 도메인과 하위 도메인으로 요청을 전송할 때만 쿠키 포함
//			c.setPath("경로"); //지정한 경로의 하위 경로로 요청을 전송할 때만 쿠키 포함. 
//							  //설정 안할 경우 지금 파일의 상위 폴더경로로 들어감("/exweb")
//			c.setHttpOnly(true); //true로 설정하면 자바스크립트로 쿠키 접근 불가
//			c.setSecure(true); //true로 설정하면, https:// 와 같은 보안프로토콜 사용시에만 쿠키 포함
//			c.setComment(rval); //쿠키에 대한 부가설명
//			c.setVersion(0); //0을 주면 인터넷 초창기부터 사용되던 쿠키를 만듬. 1은 최신의 쿠키 표준화에 맞춰서 생성. 보통 0으로 사용
